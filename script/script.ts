...
function add_book_to_table(book: typeof Book) {
    ...
    const title_cell = new_row.insertCell(0);
    title_cell.textContent = book.title;
    const author_cell = new_row.insertCell(1);
    author_cell.textContent = book.author;
    const pages_cell = new_row.insertCell(2);
    pages_cell.textContent = book.pages;
    const already_read_cell = new_row.insertCell(3);
